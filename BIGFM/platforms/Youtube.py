...
class YouTubeAPI:
        ...
        if YouTubeAPI.youtube_client:
            for _ in range(len(YOUTUBE_API_KEYS)):
                try:
                    ...
                    return v["snippet"]["title"], self.format_seconds(ts), ts, v["snippet"]["thumbnails"]["high"]["url"], vidid
                except HttpError as e:
                    if e.resp.status in [403, 429]: self._rotate_key(); continue
                    break
                except Exception: break

        # 2. Optimized Fallback (Bypassing SABR)
        try:
            loop = asyncio.get_running_loop()
            query = f"ytsearch1:{link}" if not vidid else f"https://www.youtube.com/watch?v={vidid}"
            
            # SABR fix ke liye specific extractor args
            ydl_opts = {
                "quiet": True,
                "no_warnings": True,
                ...
                # Ye line SABR error bypass karegi
                "extractor_args": {"youtube": {"player_client": ["android", "web"], "skip": ["web_safari", "ios"]}},
            }
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                return info.get('title', 'Unknown'), self.format_seconds(ts), ts, info.get('thumbnail', ''), info.get('id', '')
        except Exception as e:
            print(f"Final Error: {e}")
            return None

    # Helper functions details() use karenge
    ...
    async def video(self, link: str, videoid: Union[bool, str] = None):
        ...
        # Video download mein bhi SABR fix
        cmd = ["yt-dlp", "-g", "-f", "best[height<=?720][ext=mp4]", "--extractor-args", "youtube:player-client=android", l]
